...
class DoubleMetricLearning(GraphConstructionStage, LightningModule):
    ...
    def get_distances(self, src_embedding, tgt_embedding, pred_edges):
        reference = src_embedding[pred_edges[0]]
        neighbors = tgt_embedding[pred_edges[1]]
    ...
    def training_step(self, batch, batch_idx):
        ...
        batch.edge_index, src_embedding, tgt_embedding = self.get_training_edges(batch)
        self.apply_embedding(batch, src_embedding, tgt_embedding, batch.edge_index)
        ...
        loss = self.loss_function(batch, src_embedding, tgt_embedding, weights)
    ...
    def get_training_edges(self, batch):
        ...
        with torch.no_grad():
            src_embedding, tgt_embedding = self.apply_embedding(batch)

        query_indices, querys_src = self.get_query_points(batch, src_embedding)

        # Append Hard Negative Mining (hnm) with KNN graph
        training_edges = self.append_hnm_pairs(
            training_edges, querys_src, query_indices, tgt_embedding
        )

        # Append random edges pairs (rp) for stability
        training_edges = self.append_random_pairs(
            training_edges, query_indices, tgt_embedding
        )
        ...
        return training_edges, src_embedding, tgt_embedding

    def get_truth(self, batch, pred_edges, undirected=False):
        # Calculate truth from intersection between Prediction graph and Truth graph
        if undirected:
            pred_edges = rearrange_by_distance(batch, pred_edges)
        true_edges = batch.track_edges

        pred_edges, truth, edge_map, truth_map = graph_intersection(
            pred_edges,
            true_edges,
            return_y_pred=True,
            return_pred_to_truth=True,
            return_truth_to_pred=True,
            unique_pred=False,
            undirected=undirected,
        )

        return pred_edges, truth, edge_map, truth_map, batch.track_edges
    ...
    def apply_embedding(
        self,
        batch,
        src_embedding_inplace=None,
        tgt_embedding_inplace=None,
        training_edges=None,
    ):
        # Apply embedding to input data
        input_data = self.get_input_data(batch)
        if src_embedding_inplace is None or training_edges is None:
            return self(input_data)

        included_hits = training_edges.unique().long()
        (
            src_embedding_inplace[included_hits],
            tgt_embedding_inplace[included_hits],
        ) = self(input_data[included_hits])
    ...
    def loss_function(
        ...
        d = self.get_distances(src_embedding, tgt_embedding, pred_edges)
    ...
    def shared_evaluation(self, batch, knn_radius, knn_num):
        ...
        (
            ...
            batch.track_to_edge_map,
            true_edges,
        ) = self.get_truth(batch, batch.edge_index, undirected=self.hparams.get("undirected", False))

        batch.edge_particle_id = torch.zeros_like(batch.edge_index[0])
        batch.edge_particle_id[edge_map >= 0] = batch.track_particle_id[
            edge_map[edge_map >= 0]
        ]

        weights = self.get_weights(batch)

        d = self.get_distances(src_embedding, tgt_embedding, batch.edge_index)
        ...
        return {
            "loss": loss,
            "distances": d,
            "preds_src": src_embedding,
            "preds_tgt": tgt_embedding,

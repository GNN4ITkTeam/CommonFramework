...
from utils.processing_utils import (
    ...
    read_spacepoints,
    read_clusters,
    get_reconstructable_particles,
    truth_match_clusters,
...
def process_event(
    ...
    **kwargs
    ...
    truth_spacepoints["cluster_index_2"] = truth_spacepoints["cluster_index_2"].fillna(
        -1
    )
    ...
    if (
        not os.path.exists(
            os.path.join(outputdir, "event{:09}-truth.csv").format(evtid)
        )
        or overwrite
    ):
        opt = {"index": False}
...
if __name__ == "__main__":
    ...
    if len(files) > 1 and num_workers > 1:
        with Pool(num_workers) as p:
            process_fnc = partial(
                ...
                inputdir=inputdir,
                outputdir=outputdir,
                particles_datatypes=particles_datatypes,
                spacepoints_datatypes=spacepoints_datatypes,
                debug=args.debug,
                evtId_offset=evtId_offset,
                overwrite=overwrite,
    ...
    print(
        "Total {:,.0f} particles, out of which {:,.0f} are of interest: {:.4f}%."
        .format(

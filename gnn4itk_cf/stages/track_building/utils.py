...
def build_truth_bgraph(
    ...
    for name, (method, queries) in target_selection.items():
        ...
        if method == "range":
            ...
            raise ValueError(f"Method must be one of range, isin, notin, but got {method}")
    ...
    for name, (method, queries) in signal_selection.items():
        if method == "range":
            ...
            truth_info["is_signal"] &= torch.isin(truth_info[name], torch.as_tensor(queries))
        elif method == "notin":
            truth_info["is_signal"] &= ~ torch.isin(truth_info[name], torch.as_tensor(queries))
        else:
            raise ValueError(f"Method must be one of range, isin, notin, but got {method}")
...
def build_pred_bgraph(
    ...
    if min_hits > 0:
        ...
        _, inverse, track_size = tracks[1].unique(return_counts = True, return_inverse = True) 
        tracks = tracks[:, track_size[inverse] >= min_hits]
    
    # relabel tracks and particles:
    original_track_id, track_id, track_size = tracks[1].unique(return_counts = True, return_inverse = True)
    num_hits = event.hit_id.shape[0]
...
def match_bgraphs(
    ...
    matching = torch.stack([
        torch.as_tensor(overlaps.row),
        torch.as_tensor(overlaps.col)
    ], dim = 0).long()
...
def evaluate_tracking(
    ...
    if style == "one-way":
        ...
        matching_df["reconstructed"] = (matching_df["efficiency"] >= matching_fraction) & (matching_df["purity"] >= matching_fraction)
        matching_df["matched"] = matching_df["reconstructed"]
    elif style == "ATLAS":
        matching_df["matched"] = (matching_df["purity"] >= matching_fraction)
        matching_df["reconstructed"] = matching_df.pid.isin(matching_df.pid[matching_df["matched"]])
    else:

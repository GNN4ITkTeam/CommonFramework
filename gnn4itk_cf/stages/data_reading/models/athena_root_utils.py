...
all_branches = event_branch_names + particle_branch_names + spacepoint_branch_names \
...
def translate_branch_name(root_branch_name : str):
    ...
    if root_branch_name not in translator.keys():
        raise ValueError(f"Unknown ROOT branch name {root_branch_name}, cannot translate it.")
...
def uproot2pandaDF(branches : dict):
    ...
    for k,val in branches.items():
        ...
        if 'particleLink' in k:
            arrays[ translate_branch_name(k) ] = val[0].tolist() #special case to deal with vector<vector<>>
        else:
            arrays[ translate_branch_name(k) ] = np.asarray(val[0]) #val must have always length 1 (only one event)
...
def read_spacepoints(branches :dict):
    ...
    spacepoints["cluster_index_2"] = spacepoints["cluster_index_2"].fillna(spacepoints["cluster_index_1"]).astype("int64")
...
def read_clusters(branches : dict, particles : pd.DataFrame):
    ...
    split_pids = pd.DataFrame(
                 [
                      [c, e, b] for c, E, B in clusters[['cluster_id','subevent','barcode']].itertuples(index=False)
                      for e,b in zip(E,B)
                ],
    ...
    split_pids["particle_id"] = split_pids.merge(
         particles[["subevent", "barcode", "particle_id"]].astype({"subevent": str, "barcode": str}),
           how="left",on=["subevent", "barcode"])["particle_id"].fillna(0).astype(int)
    ...
    clusters = clusters.merge(split_pids, on="cluster_id",how='left').astype({"cluster_id": int})
...
truth_col_order=['hit_id','x','y','z','cluster_index_1','cluster_index_2','hardware','cluster_x_1','cluster_y_1','cluster_z_1',
'barrel_endcap','layer_disk','eta_module','phi_module','side_1','norm_x_1','norm_y_1','norm_z_1',
'count_1','charge_count_1',
'loc_eta_1','loc_phi_1','localDir0_1','localDir1_1','localDir2_1','lengthDir0_1','lengthDir1_1','lengthDir2_1',
'glob_eta_1','glob_phi_1','eta_angle_1','phi_angle_1','particle_id_1',
'cluster_x_2','cluster_y_2','cluster_z_2','side_2','norm_x_2','norm_y_2','norm_z_2',
'count_2','charge_count_2',
'loc_eta_2','loc_phi_2','localDir0_2','localDir1_2','localDir2_2','lengthDir0_2','lengthDir1_2','lengthDir2_2',
'glob_eta_2','glob_phi_2','eta_angle_2','phi_angle_2','particle_id_2',
'particle_id','region','module_id']

particles_col_order=['particle_id','subevent','barcode','px','py','pz','pt','eta','vx','vy','vz','radius','status','charge','pdgId','pass','vProdNIn','vProdNOut','vProdStatus','vProdBarcode','num_clusters']
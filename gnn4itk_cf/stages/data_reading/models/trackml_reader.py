...
class TrackMLReader(EventReader):
    def __init__(self, config):
        ...
        self.trainset, self.valset, self.testset = random_split(self.raw_events,
                                                                [int(len(self.raw_events) * 0.8),
                                                                 ...
                                                                 len(self.raw_events) - int(len(self.raw_events) * 0.8) - int(len(self.raw_events) * 0.1)])
        self.get_detector()
        self.get_module_lookup()

    def get_detector(self):
        # Let's get the detector
        self.detector, self.detector_dims = trackml_utils.load_detector(self.config["detector_path"])

    def get_module_lookup(self):
        # Let's get the module lookup
        self.module_lookup = self.detector.reset_index()[["index"] + self.config["module_columns"]].rename(columns={"index": "module_index"})
    ...
    def _build_single_csv(self, event, output_dir=None):
        ...
        particles.to_csv(os.path.join(output_dir, "event{:09}-particles.csv".format(int(event_id))), index=False)
    ...
    def _process_truth(self, truth, hits, cells):
        ...
        truth = truth.merge(self.module_lookup, on=self.config["module_columns"], how="left")

...
def build_signal_edges(event, weighting_config, true_edges):
    ...
    for weight_spec in weighting_config:
        ...
        signal_mask |= get_weight_mask(event, true_edges, yless_weight_spec["conditions"])
...
def handle_weighting(event, weighting_config, pred_edges=None, truth=None, true_edges=None, truth_map=None):
    ...
    if true_edges is None:
        assert "track_edges" in event.keys, "If true_edges is not provided, it must be in the event"
        true_edges = event.track_edges

    if truth_map is None:
        assert "truth_map" in event.keys, "If truth_map is not provided, it must be in the event"
    ...
    for weight_spec in weighting_config:
        ...
        weights[get_weight_mask(event, pred_edges, weight_spec["conditions"], true_edges, truth_map)] = weight_val
...
def map_value_to_edges(event, value_mask, edges, true_edges=None, truth_map=None):
    ...
    elif true_edges is not None and true_edges.shape[1] in [value_mask.shape[0], 2 * value_mask.shape[0]]:

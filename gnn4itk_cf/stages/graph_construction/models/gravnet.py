...
class GravNetMetricLearning(MetricLearning):
    ...
    def forward(self, x):
        ...
        for i, grav_conv in enumerate(self.grav_convs):
            ...
            hidden_features, spatial_edges, _, grav_fact = checkpoint(grav_conv, hidden_features, self.current_epoch)
...
class GravConv(nn.Module):
    def __init__(self, hparams, input_size=None, output_size=None):
        ...
        if "aggregation" not in hparams:
            ...
            feature_network_input = (1 + len(hparams["aggregation"]))*(self.input_size + 1)
        else:
            raise ValueError("Unknown aggregation type")
            
        self.grav_aggregation = aggr.MultiAggregation(hparams["aggregation"], mode="cat")
    ...
    def get_neighbors(self, spatial_features):
        ...
        if self.use_radius:
            radius_edges = radius_graph(spatial_features, r=self.r, max_num_neighbors=self.hparams["max_knn"], loop=self.hparams["self_loop"])
        ...
        if self.use_rand_k and self.rand_k > 0:
            random_edges = knn_graph(torch.rand(spatial_features.shape[0], 2, device=spatial_features.device), k=self.rand_k, loop=True) 
    ...
    def grav_pooling(self, spatial_features, hidden_features):
        ...
        aggregated_features = self.grav_aggregation(hidden_features[start] * d_weight.unsqueeze(1), end, dim=0, dim_size=hidden_features.shape[0])
    ...
    def r(self):
        if isinstance(self.hparams["r_train"], list):
            if len(self.hparams["r_train"]) == 2:
                return self.hparams["r_train"][0] + ( (self.hparams["r_train"][1] - self.hparams["r_train"][0]) * self.current_epoch / self.hparams["max_epochs"] )
            elif len(self.hparams["r_train"]) == 3:
                if self.current_epoch < self.hparams["max_epochs"]/2:
                    return self.hparams["r_train"][0] + ( (self.hparams["r_train"][1] - self.hparams["r_train"][0]) * self.current_epoch / (self.hparams["max_epochs"]/2) )
                else:
                    return self.hparams["r_train"][1] + ( (self.hparams["r_train"][2] - self.hparams["r_train"][1]) * (self.current_epoch - self.hparams["max_epochs"]/2) / (self.hparams["max_epochs"]/2) )
    ...
    def knn(self):
        ...
        if len(self.hparams["emb_knn"]) == 2:
            return int( self.hparams["emb_knn"][0] + ( (self.hparams["emb_knn"][1] - self.hparams["emb_knn"][0]) * self.current_epoch / self.hparams["max_epochs"] ) )
        elif len(self.hparams["emb_knn"]) == 3:
            return int(self.hparams["emb_knn"][0] + ((self.hparams["emb_knn"][1] - self.hparams["emb_knn"][0]) * self.current_epoch / (self.hparams["max_epochs"] / 2))) if self.current_epoch < self.hparams["max_epochs"] / 2 else int(self.hparams["emb_knn"][1] + ((self.hparams["emb_knn"][2] - self.hparams["emb_knn"][1]) * (self.current_epoch - self.hparams["max_epochs"] / 2) / (self.hparams["max_epochs"] / 2)))
        else:

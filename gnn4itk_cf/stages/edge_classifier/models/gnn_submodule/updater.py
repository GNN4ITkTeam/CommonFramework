...
from torch_geometric.nn import aggr, to_hetero, HeteroConv, MessagePassing
...
class NodeUpdater(MessagePassing):
    def __init__(self, aggr: str = "add", flow: str = "source_to_target", node_dim: int = -2, decomposed_layers: int = 1, **hparams):
        ...
        if self.hparams.get("concat_node"): input_size+=self.hparams['hidden']
    ...
    def update(self, agg_message, x):
        ...
        x_out = checkpoint(self.node_network, x_in, use_reentrant=False) if self.hparams.get("checkpoint") else self.node_network(x_in) 
    ...
    def forward(self, x, edge_index, edge, *args, **kwargs):
        ...
        x_dst = self.propagate(edge_index, x=x_dst, edge=edge, src_edge=src_edge, src_edge_index=src_edge_index)
...
class DirectedNodeUpdater(NodeUpdater):
    ...
    def setup_aggregation(self):
        ...
        if "aggregation" not in self.hparams:
            ...
            self.network_input_size = (1 + len(self.hparams["aggregation"]))*(self.hparams["hidden"])
        else:
            raise ValueError("Unknown aggregation type")
    ...
    def forward(
        ...
        **kwargs_dict,
        ...
        for edge_type, edge_index in edge_index_dict.items():
            ...
            out = conv(x_in, edge_index, edge, src_edge_index=src_edge_index, src_edge=src_edge, *args, **kwargs)

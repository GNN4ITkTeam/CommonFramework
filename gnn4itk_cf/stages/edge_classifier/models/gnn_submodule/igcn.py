...
class InteractionConv(MessagePassing):
    ...
    def __init__(self, 
            ...
            layernorm=True,
            batchnorm=False,
            aggr = "add", *, aggr_kwargs={}, flow: str = "source_to_target", node_dim: int = -2, decomposed_layers: int = 1, **kwargs):
    ...
    def aggregate(self, inputs: torch.Tensor, index: torch.Tensor, edge_index, x, ptr=None, dim_size=None) -> torch.Tensor:
...
class InteractionConv2(InteractionConv):
    def __init__(self, node_net_input_size, edge_net_input_size, hidden=128, n_node_net_layers=3, n_edge_net_layers=3, hidden_activation='SiLU', output_activation='SiLU', layernorm=True, batchnorm=False, aggr="add", *, aggr_kwargs={}, flow: str = "source_to_target", node_dim: int = -2, decomposed_layers: int = 1, **kwargs):
        super().__init__(node_net_input_size, hidden, n_node_net_layers, n_edge_net_layers, hidden_activation, output_activation, layernorm, batchnorm, aggr, aggr_kwargs=aggr_kwargs, flow=flow, node_dim=node_dim, decomposed_layers=decomposed_layers, **kwargs)
    ...
    def aggregate(self, inputs: torch.Tensor, index: torch.Tensor, edge_index, x, in_out_diff_agg, ptr=None, dim_size=None) -> torch.Tensor:
    ...
    def forward(self, edge_index, x, e, in_out_diff_agg=True):
        ...
        x = self.propagate(edge_index=edge_index, x=x, e=e, in_out_diff_agg=in_out_diff_agg)

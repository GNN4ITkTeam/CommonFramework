...
class HeteroNodeEncoder(torch.nn.Module):
    ...
    def forward(self, x_dict: dict):     
        for node_type, x_in in x_dict.items():
            network = partial(checkpoint, self.encoders[node_type], use_reentrant=False) if self.do_checkpoint else self.encoders[node_type]
...
class HeteroEdgeEncoder(torch.nn.Module):
    ...
    def forward(self, x_dict: dict, edge_index_dict: dict, edge_dict={}, *args, **kwargs):
        edge_dict = {}

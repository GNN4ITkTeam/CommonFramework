...
def make_mlp(
    ...
    for i in range(n_layers - 1):
        ...
        if batch_norm: # hidden_batch_norm
            layers.append(
                nn.BatchNorm1d(sizes[i + 1], eps=6e-05, track_running_stats=False, affine=True) # TODO : Set BatchNorm and LayerNorm parameters in config file ?
            )
    ...
    if output_activation is not None:
        ...
        if output_batch_norm:
            layers.append(
                nn.BatchNorm1d(sizes[-1], eps=6e-05, track_running_stats=False, affine=True) # TODO : Set BatchNorm and LayerNorm parameters in config file ?
            )

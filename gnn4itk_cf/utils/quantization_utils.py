...
from gnn4itk_cf.stages.graph_construction.models.utils import build_edges,graph_intersection
...
def quantize_features(features, verbose=False, fixed_point=False, pre_point : int = 0, post_point : int = 0):
    features = torch.clamp(features,-(2**pre_point), 2**pre_point-2**(-post_point))
...
class onnx_export(Callback):
    ...
    def on_train_epoch_end(self,trainer,pl_module):
        ...
        if trainer.current_epoch !=0:   
            ...
            if(model.hparams["quantized_network"]):
                parameters_to_prune_copy = [(model_copy[1], "weight"), (model_copy[4], "weight"), (model_copy[7], "weight"), (model_copy[10], "weight"), (model_copy[13], "weight")]
            else:
                parameters_to_prune_copy = [(model_copy[0], "weight"), (model_copy[3], "weight"), (model_copy[6], "weight"), (model_copy[9], "weight"), (model_copy[12], "weight")]
            if(model.last_pruned > -1):
                for paras in parameters_to_prune_copy:
                    prune.remove(paras[0], name = 'weight')
                    
            export_path,export_path_cleanup,export_json = self.get_onnx_paths(trainer.current_epoch)

            export_qonnx(model_copy, export_path = export_path,
                        input_t = self.input_tensor(model.hparams).to('cuda'), 
                        export_params="True") # exporting the model to calculate BOPs just before we do pruning
            ...
            inf_cost = inference_cost(export_path_cleanup, output_json = export_json, discount_sparsity = True)
            model.log_dict({"total_bops": inf_cost["total_bops"], "total_mem_w_bits": inf_cost["total_mem_w_bits"], "total_mem_o_bits": inf_cost["total_mem_o_bits"]})
    ...
    def input_tensor(self,config):
        ...
        if(config["quantized_network"]):    
            ...
            scale_tensor = torch.full((1,len(config["node_features"])),1./(input_bitwidth))
            zp = torch.tensor(0)
            signed = True
            input_tensor = QuantTensor(input_tensor, scale_tensor, zp, input_bitwidth, signed, training = False)
        else:
    ...
    def get_onnx_dir(self,trainer):
        ...
        if trainer.current_epoch !=0: ##just to avoid crashing at zero epoch
            model_checkpoint = [callback for callback in trainer.callbacks if isinstance(callback, ModelCheckpoint)][0]
            file_directory = model_checkpoint.kth_best_model_path  ##it gives path/best-version.ckpt
            file_directory = file_directory[:-5]  ##removes .ckpt extension
            onnx_directory = file_directory.replace('artifacts','onnx_exports') #replace artifact with onnx_export directory 
            if not os.path.exists(onnx_directory):  #3creating directory acc to the version
...
class auc_score(Callback):  
    ...
    def auc(self,model):
        ...
        for batch in model.testset:
            ...
            total_eff_list,total_pur_list,signal_eff_list,signal_pur_list = self.eff_and_pur(model,batch,radius_arr,model.hparams["knn_test"])
    ...
    def eff_and_pur(self,model,batch,radius_arr,knn):
        ...
        for radius in radius_arr:
            ...
            batch.edge_index = build_edges(
                    query=embedding, database=embedding, indices=None, r_max=radius, k_max = 500, backend="FRNN"
                )
            

            batch.edge_index, batch.y, batch.truth_map, true_edges = model.get_truth(batch, batch.edge_index)
        ...
        return np.array(total_eff_arr),np.array(total_pur_arr),np.array(signal_eff_arr),np.array(signal_pur_arr)

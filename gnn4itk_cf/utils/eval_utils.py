...
from gnn4itk_cf.utils.plotting_utils import get_ratio, plot_1d_histogram, plot_eff_pur_region, plot_efficiency_rz
...
def graph_construction_efficiency(lightning_module, plot_config, config):
    ...
    for true_pos_hist, true_hist, bins, xlabel, logx, filename in zip(
        ...
        fig, ax = plot_1d_histogram(hist, bins, err, xlabel, plot_config['title'], plot_config.get('ylim', [0.9, 1.04]), "Efficiency", logx=logx)
        ...
        atlasify(atlas="Internal",
                subtext=r"$\sqrt{s}=14$TeV, $t \bar{t}$, $\langle \mu \rangle = 200$, primaries $t \bar{t}$ and soft interactions) " + "\n"
                        r"$p_T > 1$GeV, $|\eta < 4$" + "\n"
                        r"Mean graph size: " + f"{np.mean(graph_size):.2e}" + r"$\pm$" + f"{np.std(graph_size):.2e}" + "\n" 
                        + f"Global efficiency: {all_y_truth.sum() / all_pt.shape[0] :.4f}"
                )
        fig.savefig(os.path.join(config["stage_dir"], filename))

        print(f'Finish plotting. Find the plot at {os.path.join(config["stage_dir"], filename)}')

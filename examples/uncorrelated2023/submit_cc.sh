...
#SBATCH --time=6:00:00
#SBATCH -o /scratch/bbhj/jareddb2/GNN4ITk/commonframework_data_output/uncorrelated2023/job_logs/job_track_cc_CTD2023.out
#SBATCH -e /scratch/bbhj/jareddb2/GNN4ITk/commonframework_data_output/uncorrelated2023/job_logs/job_track_cc_CTD2023.err
...
start=`date +%s`
g4i-infer track_building_infer_v2.yaml
end=`date +%s`
time_diff=$(($end-$start-18*3600))
echo "track_building_infer.yaml Elapsed Time: `date -d @$time_diff +%H:%M:%S`" >> /scratch/bbhj/jareddb2/GNN4ITk/commonframework_data_output/uncorrelated2023/job_logs/job_track_cc_CTD2023.out

## Track Building Eval
start=`date +%s`
g4i-eval track_building_eval_v2.yaml
end=`date +%s`
time_diff=$(($end-$start-18*3600))
echo "track_building_eval.yaml Elapsed Time: `date -d @$time_diff +%H:%M:%S`" >> /scratch/bbhj/jareddb2/GNN4ITk/commonframework_data_output/uncorrelated2023/job_logs/job_track_cc_CTD2023.out
